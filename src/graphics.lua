...
function AnimationSet:draw(sprite)
  if self.currentAnimation then
    self.currentAnimation:draw(self.image, sprite.pos.x, sprite.pos.y, 0, sprite.pos.dir, 1, sprite.dim.w / 2, 0)
  end
end
...
function load()
  ...
  animations.Slime = AnimationSet(slime, {
    ...
    idleS = anim8.newAnimation('once', g('1,3'), 0.1),
    idleE = anim8.newAnimation('once', g('1,4'), 0.1),
    walkingN = anim8.newAnimation('loop', g('1-3,1'), 0.1),
    walkingW = anim8.newAnimation('loop', g('1-3,2'), 0.1),
    walkingS = anim8.newAnimation('loop', g('1-3,3'), 0.1),
    walkingE = anim8.newAnimation('loop', g('1-3,4'), 0.1),
    })
end
